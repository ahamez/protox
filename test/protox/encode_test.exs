defmodule Protox.EncodeTest do
  ...
  test "empty", %{} do
    assert Protox.Encode.encode_binary(%Sub{}) == <<>>
  end


  test "Sub.a" do
    assert Protox.Encode.encode_binary(%Sub{a: 150})
           == <<8, 150, 1>>
  end


  test "Sub.b" do
    assert Protox.Encode.encode_binary(%Sub{b: "testing"})
           == <<18, 7, 116, 101, 115, 116, 105, 110, 103>>
  end

  test "Sub.c" do
    assert Protox.Encode.encode_binary(%Sub{c: -300}) ==\
           <<48, 212, 253, 255, 255, 255, 255, 255, 255, 255, 1>>
  end


  test "Sub.d; Sub.e" do
    assert Protox.Encode.encode_binary(%Sub{d: 901, e: 433}) ==\
           <<56, 133, 7, 64, 177, 3>>
  end


  test "Sub.f" do
    assert Protox.Encode.encode_binary( %Sub{f: -1323}) ==\
           <<72, 213, 20>>
  end


  test "Sub.g" do
    assert Protox.Encode.encode_binary(%Sub{g: [1,2]}) ==\
  ...
  test "Sub.g; Sub.h; Sub.i" do
    assert Protox.Encode.encode_binary(%Sub{g: [0], h: [-1], i: [33.2, -44.0]}) ==\
  ...
  test "Sub.h" do
    assert Protox.Encode.encode_binary(%Sub{h: [-1,-2]}) ==\
            <<114, 8, 255, 255, 255, 255, 254, 255, 255, 255>>
  end


  test "Sub.j, unpacked in definition" do
    assert Protox.Encode.encode_binary(%Sub{j: [1, 2, 3]})
           == <<128, 1, 1, 128, 1, 2, 128, 1, 3>>
  end


  test "Sub.k; Sub.l" do
    assert Protox.Encode.encode_binary(%Sub{k: 23, l: -99})
           == <<141, 1, 23, 0, 0, 0, 145, 1, 157, 255, 255, 255, 255, 255, 255, 255>>
  end


  test "Sub.m, empty" do
    assert Protox.Encode.encode_binary(%Sub{m: <<>>})
           == <<>>
  end


  test "Sub.m" do
    assert Protox.Encode.encode_binary(%Sub{m: <<1,2,3>>})
           == <<154, 1, 3, 1, 2, 3>>
  end


  test "Sub.n" do
    assert Protox.Encode.encode_binary(%Sub{n: [true, false, false, true]}) ==\
           <<162, 1, 4, 1, 0, 0, 1>>
  end


  test "Sub.n (all true)" do
    assert Protox.Encode.encode_binary(%Sub{n: [true, true, true, true]}) ==\
           <<162, 1, 4, 1, 1, 1, 1>>
  end


  test "Sub.n (all false)" do
    assert Protox.Encode.encode_binary(%Sub{n: [false, false, false, false]}) ==\
           <<162, 1, 4, 0, 0, 0, 0>>
  end


  test "Sub.o " do
    assert Protox.Encode.encode_binary(%Sub{o: [:FOO, :BAR, :FOO], z: 0}) ==\
           <<170, 1, 3, 0, 1, 0>>
  end


  test "Sub.o (unknown entry) " do
    assert Protox.Encode.encode_binary(%Sub{o: [:FOO, :BAR, :FOO, 2], z: 0}) ==\
           <<170, 1, 4, 0, 1, 0, 2>>
  end


  test "Sub.p (unpacked in definition) " do
    assert Protox.Encode.encode_binary(%Sub{p: [true, false, true, false]}) ==\
           <<176, 1, 1, 176, 1, 0, 176, 1, 1, 176, 1, 0>>
  end


  test "Sub.q (unpacked in definition) " do
    assert Protox.Encode.encode_binary(%Sub{q: [:FOO, :BAR, :BAR, :FOO]}) ==\
           <<184, 1, 0, 184, 1, 1, 184, 1, 1, 184, 1, 0>>
  end


  test "Sub.q (unpacked in definition, with unknown values) " do
    assert Protox.Encode.encode_binary(%Sub{q: [:FOO, :BAR, 2, :FOO]}) ==\
  ...
  test "Msg.f, empty" do
    assert Protox.Encode.encode_binary(%Msg{f: %Sub{}})
           == <<26, 0>>
  end


  test "Msg.f.a" do
    assert Protox.Encode.encode_binary(%Msg{f: %Sub{a: 150}})
           == <<26, 3, 8, 150, 1>>
  end
  ...
  test "Msg.j" do
    assert Protox.Encode.encode_binary(%Msg{j: [%Sub{a: 42}, %Sub{b: "foo"}]})
  ...
  test "Msg.m, Sub" do
    assert %Msg{m: {:o, %Sub{a: 42}}} |> Protox.Encode.encode_binary()
           == <<90, 2, 8, 42>>
  end
