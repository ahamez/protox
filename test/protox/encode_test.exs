defmodule Protox.EncodeTest do
  ...
  test "Sub.a" do
    assert Protox.Encode.encode(%Sub{a: 150, z: 0}) |> :binary.list_to_bin()
           == <<8, 150, 1>>
  end


  test "Sub.b" do
    assert Protox.Encode.encode(%Sub{b: "testing", z: 0}) |> :binary.list_to_bin()
           == <<18, 7, 116, 101, 115, 116, 105, 110, 103>>
  end

  test "Sub.c" do
    assert Protox.Encode.encode(%Sub{c: -300, z: 0}) |> :binary.list_to_bin()
           == <<48, 212, 253, 255, 255, 255, 255, 255, 255, 255, 1>>
  end


  test "Sub.d; Sub.e" do
    assert Protox.Encode.encode(%Sub{d: 901, e: 433, z: 0}) |> :binary.list_to_bin()
          == <<56, 133, 7, 64, 177, 3>>
  end


  test "Sub.f" do
    assert Protox.Encode.encode( %Sub{f: -1323, z: 0}) |> :binary.list_to_bin()
           == <<72, 213, 20>>
  end


  test "Sub.g" do
    assert Protox.Encode.encode(%Sub{g: [1,2], z: 0}) |> :binary.list_to_bin()
  ...
  test "Sub.g; Sub.h; Sub.i" do
    assert Protox.Encode.encode(%Sub{g: [0], h: [-1], i: [33.2, -44.0], z: 0}) |> :binary.list_to_bin()
  ...
  test "Sub.h" do
    assert Protox.Encode.encode(%Sub{h: [-1,-2], z: 0}) |> :binary.list_to_bin()
           == <<114, 8, 255, 255, 255, 255, 254, 255, 255, 255>>
  end


  test "Sub.j, unpacked in definition" do
    assert Protox.Encode.encode(%Sub{j: [1, 2, 3], z: 0}) |> :binary.list_to_bin()
           == <<128, 1, 1, 128, 1, 2, 128, 1, 3>>
  end


  test "Sub.k; Sub.l" do
    assert Protox.Encode.encode(%Sub{k: 23, l: -99, z: 0}) |> :binary.list_to_bin()
           == <<141, 1, 23, 0, 0, 0, 145, 1, 157, 255, 255, 255, 255, 255, 255, 255>>
  end


  test "Sub.m, empty" do
    assert Protox.Encode.encode(%Sub{m: <<>>, z: 0}) |> :binary.list_to_bin()
           == <<>>
  end


  test "Sub.m" do
    assert Protox.Encode.encode(%Sub{m: <<1,2,3>>, z: 0}) |> :binary.list_to_bin()
           == <<154, 1, 3, 1, 2, 3>>
  end


  test "Sub.n" do
    assert Protox.Encode.encode(%Sub{n: [true, false, false, true], z: 0}) |> :binary.list_to_bin()
           == <<162, 1, 4, 1, 0, 0, 1>>
  end


  test "Sub.n (all true)" do
    assert Protox.Encode.encode(%Sub{n: [true, true, true, true], z: 0}) |> :binary.list_to_bin()
           == <<162, 1, 4, 1, 1, 1, 1>>
  end


  test "Sub.n (all false)" do
    assert Protox.Encode.encode(%Sub{n: [false, false, false, false], z: 0}) |> :binary.list_to_bin()
           == <<162, 1, 4, 0, 0, 0, 0>>
  end


  test "Sub.o " do
    assert Protox.Encode.encode(%Sub{o: [:FOO, :BAR, :FOO], z: 0, z: 0}) |> :binary.list_to_bin()
           == <<170, 1, 3, 0, 1, 0>>
  end


  test "Sub.o (unknown entry) " do
    assert Protox.Encode.encode(%Sub{o: [:FOO, :BAR, :FOO, 2], z: 0, z: 0}) |> :binary.list_to_bin()
           == <<170, 1, 4, 0, 1, 0, 2>>
  end


  test "Sub.p (unpacked in definition) " do
    assert Protox.Encode.encode(%Sub{p: [true, false, true, false], z: 0}) |> :binary.list_to_bin()
           == <<176, 1, 1, 176, 1, 0, 176, 1, 1, 176, 1, 0>>
  end


  test "Sub.q (unpacked in definition) " do
    assert Protox.Encode.encode(%Sub{q: [:FOO, :BAR, :BAR, :FOO], z: 0}) |> :binary.list_to_bin()
           == <<184, 1, 0, 184, 1, 1, 184, 1, 1, 184, 1, 0>>
  end


  test "Sub.q (unpacked in definition, with unknown values) " do
    assert Protox.Encode.encode(%Sub{q: [:FOO, :BAR, 2, :FOO], z: 0}) |> :binary.list_to_bin()
  ...
  test "Sub.s" do
    assert Protox.Encode.encode(%Sub{s: :TWO, z: 0}) |> :binary.list_to_bin()
           == <<200, 1, 2>>
  end


  test "Sub.t, default is nil" do
    assert Protox.Encode.encode(%Sub{t: nil, z: 0}) |> :binary.list_to_bin()
           == <<>>
  end
  ...
  test "Msg.f, empty" do
    assert Protox.Encode.encode(%Msg{f: %Sub{z: 0}}) |> :binary.list_to_bin()
           == <<26, 0>>
  end


  test "Msg.f.a" do
    assert Protox.Encode.encode(%Msg{f: %Sub{a: 150, z: 0}}) |> :binary.list_to_bin()
           == <<26, 3, 8, 150, 1>>
  end
  ...
  test "Msg.j" do
    assert Protox.Encode.encode(%Msg{j: [%Sub{a: 42, z: 0}, %Sub{b: "foo", z: 0}]}) |> :binary.list_to_bin()
  ...
  test "Msg.m, Sub" do
    assert Protox.Encode.encode(%Msg{m: {:o, %Sub{a: 42, z: 0}}}) |> :binary.list_to_bin()
           == <<90, 2, 8, 42>>
  end
