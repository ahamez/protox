defmodule Protox.EncodeTest do
  ...
  test "empty", %{} do
    assert Sub.encode_binary(%Sub{}) == <<>>
  end


  test "Sub.a" do
    assert Sub.encode_binary(%Sub{a: 150})
           == <<8, 150, 1>>
  end


  test "Sub.b" do
    assert Sub.encode_binary(%Sub{b: "testing"})
           == <<18, 7, 116, 101, 115, 116, 105, 110, 103>>
  end

  test "Sub.c" do
    assert Sub.encode_binary(%Sub{c: -300}) ==\
           <<48, 212, 253, 255, 255, 255, 255, 255, 255, 255, 1>>
  end


  test "Sub.d; Sub.e" do
    assert Sub.encode_binary(%Sub{d: 901, e: 433}) ==\
           <<56, 133, 7, 64, 177, 3>>
  end


  test "Sub.f" do
    assert Sub.encode_binary( %Sub{f: -1323}) ==\
           <<72, 213, 20>>
  end


  test "Sub.g" do
    assert Sub.encode_binary(%Sub{g: [1,2]}) ==\
  ...
  test "Sub.g; Sub.h; Sub.i" do
    assert Sub.encode_binary(%Sub{g: [0], h: [-1], i: [33.2, -44.0]}) ==\
  ...
  test "Sub.h" do
    assert Sub.encode_binary(%Sub{h: [-1,-2]}) ==\
            <<114, 8, 255, 255, 255, 255, 254, 255, 255, 255>>
  end


  test "Sub.j, unpacked in definition" do
    assert Sub.encode_binary(%Sub{j: [1, 2, 3]})
           == <<128, 1, 1, 128, 1, 2, 128, 1, 3>>
  end


  test "Sub.k; Sub.l" do
    assert Sub.encode_binary(%Sub{k: 23, l: -99})
           == <<141, 1, 23, 0, 0, 0, 145, 1, 157, 255, 255, 255, 255, 255, 255, 255>>
  end


  test "Sub.m, empty" do
    assert Sub.encode_binary(%Sub{m: <<>>})
           == <<>>
  end


  test "Sub.m" do
    assert Sub.encode_binary(%Sub{m: <<1,2,3>>})
           == <<154, 1, 3, 1, 2, 3>>
  end
