defmodule Protox.RandomInit do
  ...
  defp do_generate(mod) do
    Enum.reduce(mod.defs(), struct(mod.__struct__), fn {_, {name, kind, type}}, msg ->
      case kind do
        # TODO.
        {:oneof, _} ->
          msg
        ...
        _ ->
          struct!(msg, [{name, value(kind, type)}])
      end
    end)
  end
  ...
  defp value({:default, _}, ty)
       when ty == :int32 or ty == :int64 or ty == :sint32 or ty == :sint64 or ty == :sfixed32 or
              ty == :sfixed64 do
  ...
  defp value(:unpacked, m = {:message, _}) do
    Enum.reduce(1..:rand.uniform(10), [], fn _, acc ->
